...
class DetectObstacle:
    ...
    def __init__(self, config: Any):
        ...
        self.od_env = config.env["obstacle_detection"]
        if self.od_env["missing_point_alarm"]:
    ...
    def __call__(
        ...
        missing_point_alarm = self.od_env["missing_point_alarm"]
        ...
        max_obstacle_dist = get_max_obstacle_distance(self.od_env)
        ...
        if self.detect_points is None:
            ...
            self.detect_points = init_detect_points(
                self.od_env, img_height, img_width, area, spilt_count
            )
        ...
        for i in range(len(self.detect_points)):
            ...
            for j in range(len(self.detect_points[i])):
                ...
                if dist > 0:
                    ...
                    if is_hole(self.od_env, height):
                        color = (0, 0, 255)
                        if dist < min_hole_distance:
                            min_hole_distance = dist
                    # 如果高度低於身高，又高於最低障礙物的判斷標準，則代表有障礙物
                    elif is_obstacle(self.od_env, height, dist):
                        color = (0, 255, 255)
                        if dist < min_obstacle_distance:
                            min_obstacle_distance = dist
                    elif (
                        debug
                        and self.od_env["lowest_obstacle_height"]
                        > height
                        > self.od_env["highest_hole_height"]
                    ):
                ...
                if debug:
                    ...
                    color_img = draw_text(
                        color_img, str(dist), (x, y - 10), (255, 200, 255), font_scale
                    )
        ...
        if Alarm.instance is not None and self.alarm:
            alert(self.od_env, min_hole_distance, min_obstacle_distance, missing_point)
